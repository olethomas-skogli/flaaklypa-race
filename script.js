...
const startLeaderboardStatus = document.getElementById(
  "startLeaderboardStatus"
);

// Professional Cycle Progress Bar Elements (Integrated in Unified Scoreboard)
const cycleProgressContainer = document.getElementById(
...
if (
  ...
  console.log(
    "📱 Firebase config not found or not configured, using localStorage for leaderboard"
  );
  console.log(
    "💡 To enable online leaderboard, edit firebase-config.js with your Firebase settings"
  );
  firebaseEnabled = false;
  if (leaderboardStatus) {
    leaderboardStatus.innerHTML = "📱 Local Leaderboard";
...
function unlockAudio() {
  if (!audioUnlocked) {
    ...
    const unlockSound = new Howl({
      src: [
        "data:audio/wav;base64,",
      ],
      volume: 0,
    });
    unlockSound.play();
...
const milestoneSound = new Howl({
  ...
  onload: function () {
    console.log(
      "✅ Milestone sound (schwinge-i-sweiva.mp3) loaded successfully"
    );
  },
  onloaderror: function (id, error) {
...
// Sabotage sound for mist effect with professional state management
let sabotageSound;
let sabotagePlayId = null; // Track current playback instance
let sabotagePlayingState = false; // Track if sabotage should be playing

// Professional audio error handling wrapper
function safeAudioOperation(
  audioObject,
  operation,
  description,
  fallback = null
  ...
  try {
    if (!audioObject) {
      console.warn(`⚠️ ${description}: Audio object is null/undefined`);
      return fallback;
    }

    const result = operation(audioObject);
    console.log(`✅ ${description}: Operation completed successfully`);
    return result;
  } catch (error) {
    console.error(`❌ ${description}: Operation failed -`, error);
    ...
    if (fallback && typeof fallback === "function") {
      try {
        ...
        console.error(
          `❌ ${description}: Fallback also failed -`,
          fallbackError
        );
      }
    }
...
// Professional audio state management function
function manageSabotageSound(action, reason = "") {
  const timestamp = Date.now();
  console.log(`🎵 [${timestamp}] SABOTAGE SOUND: ${action} - ${reason}`);

  try {
    if (action === "play") {
      // Ensure we're not already playing
      if (sabotagePlayingState && sabotageSound && sabotageSound.playing()) {
        console.log(
          "🔄 Sabotage sound already playing, skipping duplicate play request"
        );
        return;
      }

      // Stop any existing playback first
      if (sabotagePlayId !== null) {
        sabotageSound.stop(sabotagePlayId);
        sabotagePlayId = null;
      }

      sabotagePlayingState = true;

      if (sabotageSound && typeof sabotageSound.play === "function") {
        if (sabotageSound.state() === "loaded") {
          // Lower background music using safe audio operation
          safeAudioOperation(
            backgroundMusic,
            (audio) => audio.volume(0.05),
            "Lower background music for sabotage"
          );

          sabotagePlayId = safeAudioOperation(
            sabotageSound,
            (audio) => audio.play(),
            "Play sabotage sound",
            () => {
              sabotagePlayingState = false;
              return null;
            }
          );
          ...
          if (sabotagePlayId !== null) {
            console.log(
              `✅ [${timestamp}] Sabotage sound playing - ID: ${sabotagePlayId}`
            );

            // Verify playback after short delay
            setTimeout(() => {
              if (sabotageSound.playing() && sabotagePlayingState) {
                console.log(
                  `✅ [${timestamp + 100}] Sabotage sound confirmed playing`
                );
              } else if (sabotagePlayingState) {
                console.error(
                  `❌ [${
                    timestamp + 100
                  }] Sabotage sound failed to start despite play() call`
                );
                sabotagePlayingState = false;
              }
            }, 100);
          ...
          console.warn(
            `⚠️ [${timestamp}] Sabotage sound not loaded (state: ${sabotageSound.state()}), attempting anyway...`
          );

          safeAudioOperation(
            backgroundMusic,
            (audio) => audio.volume(0.05),
            "Lower background music (unloaded sabotage)"
          );

          sabotagePlayId = safeAudioOperation(
            sabotageSound,
            (audio) => audio.play(),
            "Play sabotage sound (unloaded)",
            () => {
              sabotagePlayingState = false;
              return null;
            }
        ...
        console.error(`❌ [${timestamp}] Invalid sabotage sound object`);
        sabotagePlayingState = false;
      }
    } else if (action === "stop") {
      sabotagePlayingState = false;

      if (sabotageSound && typeof sabotageSound.stop === "function") {
        if (sabotagePlayId !== null) {
          safeAudioOperation(
            sabotageSound,
            (audio) => audio.stop(sabotagePlayId),
            `Stop sabotage sound (ID: ${sabotagePlayId})`
          );
          sabotagePlayId = null;
        } else {
          safeAudioOperation(
            sabotageSound,
            (audio) => audio.stop(),
            "Stop all sabotage sound instances"
          );
        }

        // Restore background music volume using safe operation
        safeAudioOperation(
          backgroundMusic,
          (audio) => audio.volume(0.2),
          "Restore background music volume after sabotage"
        );

        console.log(
          `✅ [${timestamp}] Sabotage sound stopped successfully - ${reason}`
        );
      } else {
        console.warn(
          `⚠️ [${timestamp}] Cannot stop sabotage sound - invalid object`
        );
      }
    }
  } catch (e) {
    console.error(
      `❌ [${timestamp}] Critical error in manageSabotageSound(${action}):`,
      e
    );
    sabotagePlayingState = false;
    sabotagePlayId = null;
    // Attempt to restore background music volume on error using safe operation
    safeAudioOperation(
      backgroundMusic,
      (audio) => audio.volume(0.2),
      "Emergency restore background music volume",
      () => console.log("Background music volume restore fallback completed")
    );
  }
}
...
try {
  sabotageSound = new Howl({
    ...
    onplay: function (id) {
      console.log(`🔊 Sabotage sound started playing - ID: ${id}`);
    },
    onplayerror: function (id, error) {
    ...
    onstop: function (id) {
      console.log(`⏹️ Sabotage sound stopped - ID: ${id}`);
      if (sabotagePlayId === id) {
        sabotagePlayId = null;
        sabotagePlayingState = false;
      }
    },
  ...
  sabotageSound = {
    play: () => {
      console.warn("Sabotage sound fallback - no audio");
      return null;
    },
    stop: () => {},
    state: () => "unloaded",
...
try {
  desertSound = new Howl({
    ...
    onload: function () {
      console.log(
        "✅ Desert sound (flaklypa-stille-som-orken.mp3) loaded successfully"
      );
      console.log(
        "🎵 Desert sound duration:",
        desertSound.duration(),
        "seconds"
      );
    },
    onloaderror: function (id, error) {
  ...
  aladdinOilSound = new Howl({
    ...
    onload: function () {
      ...
      console.log(
        "🎵 Aladdin oil sound duration:",
        aladdinOilSound.duration(),
        "seconds"
      );
    },
    onloaderror: function (id, error) {
    ...
    onplay: function () {
      console.log(
        "🔊 Aladdin oil sound started playing (background audio, volume 0.15)"
      );
    },
    onplayerror: function (id, error) {
...
function areDesertSoundsReady() {
  ...
  console.log(
    "🎵 Desert sounds ready check - Desert:",
    desertReady,
    "Aladdin:",
    aladdinReady
  );
...
try {
  winterSound = new Howl({
    ...
    onload: function () {
      console.log(
        "✅ Winter sound (de-bygger-il-tempo.mp3) loaded successfully"
      );
      console.log(
        "🎵 Winter sound duration:",
        winterSound.duration(),
        "seconds"
      );
    },
    onloaderror: function (id, error) {
...
try {
  thunderSound = new Howl({
    ...
    onload: function () {
      ...
      console.log(
        "🎵 Thunder sound duration:",
        thunderSound.duration(),
        "seconds"
      );
    },
    onloaderror: function (id, error) {
...
try {
  tempoTempoSound = new Howl({
    ...
    onload: function () {
      ...
      console.log(
        "🎵 Tempo-tempo sound duration:",
        tempoTempoSound.duration(),
        "seconds"
      );
    },
    onloaderror: function (id, error) {
...
// Professional test sabotage sound function (for debugging)
function testSabotageSound() {
  console.log("🧪 TESTING SABOTAGE SOUND");
  console.log(
    "🔍 Sound state:",
    sabotageSound ? sabotageSound.state() : "undefined"
  );

  if (sabotageSound && typeof sabotageSound.play === "function") {
    const testPlayId = safeAudioOperation(
      sabotageSound,
      (audio) => audio.play(),
      "Test sabotage sound playback",
      () => console.log("Test play fallback completed")
    );

    if (testPlayId !== null) {
      console.log(`✅ Test play initiated with ID: ${testPlayId}`);
    } else {
      console.warn("⚠️ Test play returned null ID");
    }
  } else {
...
function testMilestoneSound() {
  ...
  console.log(
    "🔍 Sound state:",
    milestoneSound ? milestoneSound.state() : "undefined"
  );
...
function updateLeaderboardList(listElement) {
  ...
  leaderboard.forEach((entry, index) => {
    const li = document.createElement("li");
    li.textContent = `${index + 1}. ${entry.name} (${entry.car}): ${
      entry.score
    }`;
    listElement.appendChild(li);
  });
...
// Professional progress bar update with optimization and cycle 1 validation
let lastProgressUpdate = 0;
let lastProgressValue = -1;
const PROGRESS_UPDATE_THROTTLE = 50; // Update max every 50ms for smooth animation
...
function updateCycleProgressBar() {
  ...
  // Only update if progress has changed significantly (reduces DOM manipulation)
  const roundedProgress = Math.round(progress);
  if (
    Math.abs(roundedProgress - lastProgressValue) >= 1 ||
    currentCycle === 1
  ) {
    lastProgressValue = roundedProgress;

    // Update progress bar elements with cycle 1 specific enhancements
    cycleProgressLabel.textContent = `Cycle ${currentCycle} Progress`;
    cycleProgressPercent.textContent = `${roundedProgress}%`;
    cycleProgressFill.style.width = `${progress}%`;

    // Ensure smooth transitions for cycle 1
    if (currentCycle === 1) {
      cycleProgressFill.style.transition = "width 0.2s ease-out";
    }

    // Update current scene text with speed info
    const speedInfo = `Speed: ${currentSpeedMultiplier.toFixed(
      2
    )}x / ${cycleMaxSpeed}x max`;
    cycleProgressText.textContent = `${currentScene} Scene - ${speedInfo}`;

    // Update stage indicators with proper validation
    if (progressStages && progressStages.length > 0) {
      progressStages.forEach((stage, index) => {
        if (stage && typeof stage.classList !== "undefined") {
          stage.classList.toggle("active", index === stageIndex);
        }
      });
    }

    // Enhanced logging for cycle 1 debugging
    if (currentCycle === 1 && Math.random() < 0.01) {
      // More frequent logging for cycle 1
      console.log(
        `🎮 Cycle 1 Progress: ${roundedProgress}% | ${currentScene} | Speed: ${currentSpeedMultiplier.toFixed(
          2
        )}x | Time: ${Math.floor(gameTime / 1000)}s`
      );
    } else if (Math.random() < 0.005) {
      // Regular logging for other cycles
      console.log(
        `🎮 Cycle ${currentCycle} Progress: ${roundedProgress}% | ${currentScene} | Speed: ${currentSpeedMultiplier.toFixed(
          2
        )}x`
      );
    }
...
function resizeCanvas() {
  ...
  if (gameStarted) {
    ...
    player.x = Math.max(
      0,
      Math.min(player.x, canvasDisplayWidth - player.width)
    );
...
const WINTER_GAME_SPEED = 0.7; // Very slow game speed during winter
...
function updateGameSpeed() {
  ...
  const totalSpeedIncrease =
    pointSpeedIncrease + timeSpeedIncrease + extraSpeedBoost;
  ...
  const baseTrackSpeedWithMultiplier =
    BASE_TRACK_SPEED * currentSpeedMultiplier;
  ...
  const basePlayerSpeedWithMultiplier =
    BASE_PLAYER_SPEED * currentSpeedMultiplier;
...
function handleTap(side) {
  ...
  if (side === "left" && player.x > 0) {
    ...
    player.x = Math.min(
      canvasDisplayWidth - player.width,
      player.x + tapDistance
    );
  } else if (side === "up" && player.y > 0) {
    player.y = Math.max(0, player.y - tapDistance);
  } else if (
    side === "down" &&
    player.y < canvasDisplayHeight - player.height
  ) {
    player.y = Math.min(
      canvasDisplayHeight - player.height,
      player.y + tapDistance
    );
  }
      ...
      if (side === "left") {
        ...
        if (elementType === "stand") {
          ...
          const availableSpace =
            canvasDisplayWidth - trackRight - width - safetyMargin;
          x =
            availableSpace > 0
              ? trackRight + safetyMargin + Math.random() * availableSpace
              : trackRight + safetyMargin;
        }
      }
...
function handleSpeedBasedRadarSpawning() {
  if (mistActive && !radarActive) {
    ...
    const speedAdjustedInterval =
      baseRadarInterval / Math.max(speedMultiplier, 1);
    ...
    if (gameTime - lastRadarSpawnTime >= finalInterval) {
      ...
      if (!firstRadarCompleted || (firstRadarCompleted && mistTimeBased)) {
        ...
        if (radarSpawnCount < maxRadarsPerMist) {
          ...
          console.log(
            `🚀 Speed-based radar spawn #${radarSpawnCount} (Speed: ${speedMultiplier.toFixed(
              1
            )}x, Interval: ${finalInterval.toFixed(0)}ms)`
          );
        }
      }
    }
...
function resetGame(preserveCounters = false) {
  ...
  try {
    radarSound.stop();
    manageSabotageSound("stop", "game restart");
    ...
    console.log(
      "✅ All game sounds stopped - game reset (background music restored)"
    );
  } catch (e) {
...
function restartCycle() {
  ...
  // Ensure radar audio stops when a new cycle starts
  try {
    if (radarSound && typeof radarSound.stop === "function") {
      // Only stop if it's currently playing (Howler provides playing())
      if (radarSound.playing && radarSound.playing()) {
        radarSound.stop();
        console.log("⏹️ Radar sound stopped for new cycle");
      } else {
        // Safe stop even if it's not playing
        radarSound.stop();
      }
    }
  } catch (e) {
    console.warn("Error stopping radar sound on cycle restart:", e);
  }

  try {
    if (typeof manageSabotageSound === "function") {
      manageSabotageSound("stop", "cycle restart");
      console.log("⏹️ Sabotage sound stopped for new cycle");
    }
  } catch (e) {
    console.warn("Error stopping sabotage sound on cycle restart:", e);
  }
  ...
  const cycleMaxSpeed = getCycleMaxSpeed(currentCycle);
  console.log(
    `✅ Cycle ${currentCycle} started! Max speed: ${cycleMaxSpeed}x | Current: ${currentSpeedMultiplier}x (Score: ${score})`
  );
  console.log(
    `🔄 Scene flags reset: firstRadarCompleted=${firstRadarCompleted}, firstWinterCompleted=${firstWinterCompleted}`
  );
...
async function update(timestamp) {
  ...
  if (gameOver) {
    ...
    radarSound.stop();
    manageSabotageSound("stop", "game over");
    ...
    try {
      ...
      console.warn(
        "Error stopping winter/thunder/tempo-tempo sound on game over:",
        e
      );
    }
  ...
  if (timeBonusActive && gameTime - lastTimeBonusAwarded >= timeBonusInterval) {
    ...
    console.log(
      `⏱️ Time bonus awarded! +${timeBonusPoints} points (Total: ${score})`
    );
  }
  ...
  if (
    !desertSceneActive &&
    !winterSceneActive &&
    timestamp - lastObstacleTime > obstacleFrequency
  ) {
    createObstacle();
  ...
  if (
    !desertSceneActive &&
    !winterSceneActive &&
    timestamp - lastCollectibleTime > currentCollectibleFrequency
  ) {
    createCollectible();
  ...
  if (
    desertSceneActive &&
    timestamp - lastDesertCollectibleTime > desertCollectibleFrequency
  ) {
  ...
  if (
    !desertSceneActive &&
    !(winterSceneActive && !winterFogActive) &&
    timestamp - lastSideObstacleTime > sideObstacleFrequency
  ) {
    createSideElements();
  ...
  if (
    desertSceneActive &&
    timestamp - lastDesertObstacleTime > desertObstacleFrequency
  ) {
  ...
  if (
    desertSceneActive &&
    timestamp - lastDesertSideObstacleTime > desertSideObstacleFrequency
  ) {
  ...
  const obstacleSpawnReady =
    winterObstacleSpawnStartTime > 0 &&
    Date.now() - winterObstacleSpawnStartTime >= winterObstacleSpawnDelay;
  if (
    winterSceneActive &&
    winterFogActive &&
    obstacleSpawnReady &&
    timestamp - lastWinterObstacleTime > winterObstacleFrequency
  ) {
  ...
  if (
    ...
    obstacleSpawnReady &&
    currentSeasonPhase === "winter" &&
    timestamp - lastWinterCollectibleTime > winterCollectibleFrequency
  ) {
  ...
  if (
    ...
    timestamp - lastWrenchCollectibleTime > wrenchCollectibleFrequency
  ) {
  ...
  if (
    winterSceneActive &&
    winterFogActive &&
    timestamp - lastWinterSideObstacleTime > winterSideObstacleFrequency
  ) {
  ...
  if (desertSceneActive) {
    ...
    for (let i = desertSideObstacles.length - 1; i >= 0; i--) {
      if (
        desertSideObstacles[i].y >
        canvas.height + desertSideObstacles[i].height
      ) {
  ...
  if (winterSceneActive) {
    ...
    for (let i = winterSideObstacles.length - 1; i >= 0; i--) {
      if (
        winterSideObstacles[i].y >
        canvas.height + winterSideObstacles[i].height
      ) {
  ...
  collectibles.forEach((collectible, index) => {
    if (
      ...
      if (collectible.isRadar) {
        ...
        manageSabotageSound("stop", "radar collected");
        ...
        // Apply Ludvig 3x multiplier if active
        if (ludvigActive) {
          points *= 3;
        }
  ...
  if (
    ...
    // Play sabotage sound during mist using professional audio management
    manageSabotageSound("play", "mist triggered");
  }
  ...
  if (mistActive) {
    ...
    if (mistTimer <= 0) {
      ...
      if (!firstRadarCollected) {
        missedRadarAttempts++;
        console.log(
          `Mist ended without radar collection - missed attempts: ${missedRadarAttempts}/${maxMissedRadarAttempts}`
        );
      }
  ...
  if (radarActive) {
    ...
    if (radarTimer <= 0) {
      ...
      if (!firstRadarCompleted) {
        ...
        console.log(
          "⏱️ Time bonus system activated - 20 points every 10 seconds!"
        );
        ...
        console.log(
          '🏜️ Desert message triggered: "Her væra like stille som den ørken"'
        );
  ...
  if (
    !firstRadarCompleted &&
    !desertSceneActive &&
    missedRadarAttempts >= maxMissedRadarAttempts
  ) {
    console.log(
      `🏜️ Alternative desert scene trigger - player missed ${missedRadarAttempts} radar attempts`
    );
    ...
    lastTimeBonusAwarded = gameTime; // Start timing from now
    console.log(
      "⏱️ Time bonus system activated - 20 points every 10 seconds! (alternative trigger)"
    );
    ...
    console.log(
      "🏜️ ALTERNATIVE DESERT SCENE ACTIVATED - helping player progress"
    );
    ...
    try {
      backgroundMusic.stop();
      console.log(
        "🏜️ Alternative desert scene activated - background music stopped"
      );
    } catch (e) {
      console.warn(
        "Error stopping background music for alternative desert scene:",
        e
      );
    }
  }
  ...
  if (desertSceneActive) {
    ...
    if (!desertSoundStarted && desertSceneTimer <= desertSceneDuration - 500) {
      ...
      try {
        ...
        if (desertSound && desertSound.state() === "loaded") {
          desertSound.play();
          console.log(
            "🏜️ Primary desert sound started (0.5 second delay) - volume 1.0"
          );
        } else {
          console.warn(
            "⚠️ Primary desert sound not ready, state:",
            desertSound ? desertSound.state() : "undefined"
          );
        ...
        if (aladdinOilSound && aladdinOilSound.state() === "loaded") {
          ...
          console.log(
            "🏜️ Background aladdin sound started (0.5 second delay) - volume 0.15"
          );
        } else {
          console.warn(
            "⚠️ Background aladdin sound not ready, state:",
            aladdinOilSound ? aladdinOilSound.state() : "undefined"
          );
        ...
        if (backgroundMusic && backgroundMusic.state() !== "loaded") {
          console.log(
            "🎵 Preloading background music during desert scene for instant resume"
          );
        ...
        try {
          ...
          console.error(
            "❌ Fallback failed for primary desert sound:",
            fallbackError
          );
        }
        try {
          ...
          console.error(
            "❌ Fallback failed for background aladdin sound:",
            fallbackError
          );
        }
      }
    ...
    if (desertSceneTimer <= 0) {
      desertSceneActive = false;
      console.log(
        `🏜️ Desert scene completed (Cycle: ${currentCycle}, Score: ${score})`
      );
      ...
      if (!mistTimeBased) {
        ...
        console.log(
          "Desert scene completed - switching to time-based mist system (mist will start immediately, then every 8 seconds)"
        );
      }

      // CRITICAL: Ensure sabotage sound is properly stopped before transition to time-based mist
      // This prevents audio desync when switching from score-based to time-based mist system
      if (sabotagePlayingState || (sabotageSound && sabotageSound.playing())) {
        manageSabotageSound("stop", "desert scene transition cleanup");
        console.log(
          "🏜️ Desert transition: Cleaned up sabotage sound before mist system switch"
        );
      }
      ...
      console.log(
        "🏜️ Desert scene ended - speeds restored to pre-desert values"
      );
      ...
      try {
        ...
        if (backgroundMusic) {
          if (backgroundMusic.state() === "loaded") {
            backgroundMusic.play();
            console.log(
              "🎵 Background music resumed instantly (preloaded) - state:",
              backgroundMusic.state()
            );
          } else {
            console.warn(
              "⚠️ Background music not preloaded, attempting to load and play - state:",
              backgroundMusic.state()
            );
        ...
        console.error(
          "❌ Critical error restoring audio after desert scene:",
          e
        );
        ...
        try {
          ...
          console.error(
            "❌ Fallback failed stopping desert sound:",
            fallbackError
          );
        }
        try {
          ...
          console.error(
            "❌ Fallback failed stopping aladdin sound:",
            fallbackError
          );
        }
        try {
          ...
          console.error(
            "❌ Fallback failed starting background music:",
            fallbackError
          );
        }
      }
  ...
  if (!winterSceneActive && !firstWinterCompleted) {
    ...
    if (
      ...
      console.log(
        `❄️ Winter scene trigger - Primary: after desert completion, no mist/radar active (Cycle: ${currentCycle}, Score: ${score})`
      );
    }

    // Alternative trigger: 36 seconds + 180 points
    if (gameTime >= 36000 && score >= 180) {
      // 36 seconds = 36000ms
      shouldTriggerWinter = true;
      console.log(
        `❄️ Winter scene trigger - Alternative: 36 seconds + 180 points reached (Cycle: ${currentCycle}, Time: ${Math.floor(
          gameTime / 1000
        )}s)`
      );
    }
    ...
    if (shouldTriggerWinter) {
      ...
      console.log(
        "⚠️ PRE-WINTER WARNING ACTIVATED - Starting 2-second speed decrease"
      );
    }
  }
  ...
  if (preWinterWarningActive) {
    ...
    currentPlayerSpeed =
      preWinterOriginalPlayerSpeed -
      (preWinterOriginalPlayerSpeed - targetPlayerSpeed) * warningProgress;
    currentTrackSpeed =
      preWinterOriginalTrackSpeed -
      (preWinterOriginalTrackSpeed - targetTrackSpeed) * warningProgress;
    ...
    if (preWinterWarningTimer <= 0) {
      ...
      try {
        if (backgroundMusic && backgroundMusic.playing()) {
          backgroundMusic.stop();
          console.log(
            "❄️ Background music stopped immediately for winter scene"
          );
        }
      ...
      try {
        if (winterSound && winterSound.state() === "loaded") {
          ...
          console.warn(
            "⚠️ Winter sound not ready, state:",
            winterSound ? winterSound.state() : "undefined"
          );
  ...
  if (winterSceneActive) {
    ...
    const timeElapsedInWinter =
      winterSceneDuration + winterVisualDelay - winterSceneTimer;
    ...
    if (!winterFogActive && winterSceneTimer <= winterSceneDuration) {
      ...
      console.log(
        "❄️ Winter visual effects activated - speeds remain at start level, natural progression resumes"
      );
    }
    ...
    const timeElapsed =
      winterSceneDuration + winterVisualDelay - winterSceneTimer;
    if (
      timeElapsed >= winterPhaseDuration + springPhaseDuration &&
      currentSeasonPhase !== "summer"
    ) {
      currentSeasonPhase = "summer";
      console.log("☀️ Summer phase activated");
    } else if (
      timeElapsed >= winterPhaseDuration &&
      currentSeasonPhase === "winter"
    ) {
      currentSeasonPhase = "spring";
    ...
    if (winterSceneTimer <= 0) {
      ...
      console.log(
        "❄️ Winter scene ended - speeds continue natural progression from winter levels"
      );
      ...
      if (!cycleCompletionPending && firstWinterCompleted) {
        ...
        console.log(
          `🔄 Cycle ${currentCycle} completed! Starting cycle ${
            currentCycle + 1
          } with max speed ${nextCycleMaxSpeed}x`
        );
        console.log(
          `💰 Cycle completion bonus: +50 points! Total score: ${score}`
        );
        console.log(
          `📊 Cycle ${currentCycle} stats - Score: ${score}, Game Time: ${Math.floor(
            gameTime / 1000
          )}s`
        );
      ...
      try {
        ...
        if (backgroundMusic && backgroundMusic.state() === "loaded") {
          ...
          console.warn(
            "⚠️ Background music not ready, attempting to load and play"
          );
          if (backgroundMusic) {
            backgroundMusic.once("load", () => {
              backgroundMusic.play();
  ...
  obstacles.forEach((obstacle) => {
    ...
    const obstacleCollisionX =
      obstacle.x + (obstacle.width * collisionReduction) / 2;
    const obstacleCollisionY =
      obstacle.y + (obstacle.height * collisionReduction) / 2;
    ...
    const playerCollisionX =
      player.x + (player.width * playerCollisionReduction) / 2;
    const playerCollisionY =
      player.y + (player.height * playerCollisionReduction) / 2;
    const playerCollisionWidth = player.width * (1 - playerCollisionReduction);
    const playerCollisionHeight =
      player.height * (1 - playerCollisionReduction);
  ...
  if (desertSceneActive) {
    desertObstacles.forEach((obstacle) => {
      ...
      const obstacleCollisionY =
        obstacle.y + (obstacle.height * collisionReduction) / 2;
      const obstacleCollisionWidth = obstacle.width * (1 - collisionReduction);
      const obstacleCollisionHeight =
        obstacle.height * (1 - collisionReduction);
      ...
      const playerCollisionWidth =
        player.width * (1 - playerCollisionReduction);
      const playerCollisionHeight =
        player.height * (1 - playerCollisionReduction);
    ...
    desertSideObstacles.forEach((obstacle) => {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "Error playing desert side obstacle game over sound:",
            e
          );
        }
      }
  ...
  const speedMultiplier = (currentTrackSpeed / BASE_TRACK_SPEED).toFixed(1);
...
function updateRain(deltaTime) {
  ...
  if (
    now - lastRainSpawnTime > rainSpawnRate &&
    rainDrops.length < maxRainDrops
  ) {
    const canvasDisplayWidth = canvas.getBoundingClientRect().width;
    const canvasDisplayHeight = canvas.getBoundingClientRect().height;
...
function updateSun(deltaTime) {
  ...
  if (
    now - lastSunSpawnTime > sunSpawnRate &&
    sunParticles.length < maxSunParticles
  ) {
    const canvasDisplayWidth = canvas.getBoundingClientRect().width;
    const canvasDisplayHeight = canvas.getBoundingClientRect().height;
...
function drawSun() {
  if (
    !(winterSceneActive && currentSeasonPhase === "summer") ||
    sunParticles.length === 0
  )
    return;
  ...
  sunParticles.forEach((particle) => {
    ...
    gradient.addColorStop(
      0,
      `rgba(255, 223, 0, ${particle.opacity * particle.life})`
    ); // Golden center
    gradient.addColorStop(
      1,
      `rgba(255, 165, 0, ${particle.opacity * particle.life * 0.3})`
    ); // Orange edge
...
function draw() {
  ...
  for (let i = 0; i < displayHeight; i += 60) {
    ctx.fillRect(displayWidth / 2 - 10, (i + trackY) % displayHeight, 20, 40);
  }
  ...
  if (desertSceneActive && sandDunesImageLoaded) {
    ...
    ctx.drawImage(sandDunesImage, leftDunesX, 0, leftDunesWidth, displayHeight);
  ...
  obstacles.forEach((obstacle) => {
    ...
    if (obstacle.type === "desperados" && desperadosObstacleImageLoaded) {
      ...
      ctx.fillStyle =
        obstacle.type === "desperados"
          ? "purple"
          : obstacle.type === "reodor"
          ? "orange"
          : "red";
  ...
  collectibles.forEach((collectible) => {
    if (!collectible.isRadar) {
      ...
      if (collectible.type === "gold") {
        ...
        const glowRadius =
          Math.max(collectible.width, collectible.height) / 2 + 12;
  ...
  if (showMessage && !desertSceneActive && !mistActive) {
    ...
    const imageHeight = isMobileDevice
      ? Math.min(80, displayHeight * 0.12)
      : 120;

    // Responsive font size: smaller on mobile, larger on desktop
    const fontSize = isMobileDevice
      ? Math.max(16, Math.min(24, displayWidth * 0.04))
      : 38;
    ...
    if (milestoneImageLoaded) {
      ctx.drawImage(milestoneImage, imageX, imageY, imageWidth, imageHeight);
    } else {
  ...
  if (showDesertMessage) {
    ...
    const fontSize = isMobileDevice
      ? Math.max(13, Math.min(21, (displayWidth * 0.05) / 1.5))
      : 32;
    ctx.font = `${fontSize}px Arial`;
  ...
  if (showWinterHint) {
    ...
    const fontSize = isMobileDevice
      ? Math.max(18, Math.min(24, displayWidth * 0.045))
      : 28;
    ctx.font = `bold ${fontSize}px Arial`;
    ...
    if (kjelkeImageLoaded) {
      ...
      ctx.drawImage(kjelkeImage, imageX, imageY, imageWidth, imageHeight);
    } else {
      // Fallback if image fails to load
  ...
  if (mistActive) {
    ...
    for (let layer = 0; layer < layerCount; layer++) {
      ...
      for (let i = 0; i < particlesPerLayer; i++) {
        ...
        const x =
          trackLeft +
          ((i * (trackWidth / particlesPerLayer) +
            Math.sin(time * layerSpeed + i) * 30) %
            trackWidth);
        // Enhanced Y distribution for full track coverage on mobile/tablet
        const yRange =
          isMobile() || isTablet() ? canvas.height + 200 : canvas.height + 150;
        const yOffset = isMobile() || isTablet() ? -100 : -75;
        const y =
          ((i * 60 + Math.cos(time * layerSpeed * 0.8 + i) * 40) % yRange) +
          yOffset;

        // Create track-focused mist particles
        const mistSizeMultiplier = isTablet() ? 2.5 : isMobile() ? 1.8 : 1.2; // Adjusted for track coverage
        const baseRadius =
          (trackWidth / particlesPerLayer) * 0.8 * mistSizeMultiplier; // Size based on track width
    ...
    for (let layer = 0; layer < layerCount; layer++) {
      ...
      for (let i = 0; i < particlesPerLayer; i++) {
        ...
        const x =
          trackLeft +
          ((i * (trackWidth / particlesPerLayer) +
            Math.sin(time * layerSpeed + i) * 20) %
            trackWidth);
        // Winter fog Y distribution
        const yRange =
          isMobile() || isTablet() ? canvas.height + 150 : canvas.height + 100;
        const yOffset = isMobile() || isTablet() ? -75 : -50;
        const y =
          ((i * 50 + Math.cos(time * layerSpeed * 0.7 + i) * 30) % yRange) +
          yOffset;

        // Seasonal fog particles
        const particleRadius =
          isMobile() || isTablet() ? 15 + layer * 3 : 12 + layer * 2.5;
  ...
  if (showMistMessage) {
    ...
    // Responsive font size: smaller on mobile, larger on desktop
    const fontSize = isMobileDevice
      ? Math.max(20, Math.min(32, displayWidth * 0.05))
      : 48;
    ctx.font = `${fontSize}px Arial`;
    ...
    if (mistMessageImageLoaded) {
      ctx.drawImage(mistMessageImage, imageX, imageY, imageWidth, imageHeight);
    } else {
      // Fallback if image fails to load
  ...
  if (showRadarHint) {
    ...
    const fontSize = isMobileDevice
      ...
      : isTabletDevice
      ? Math.max(24, Math.min(32, displayWidth * 0.04))
      : 28;
    ...
    const imageSize = isMobileDevice
      ...
      : isTabletDevice
      ? Math.max(35, Math.min(45, displayWidth * 0.05))
      : 45;
    ...
    if (radarImageLoaded) {
      ctx.drawImage(radarImage, imageX, imageY, imageSize, imageSize);
    } else {
...
backButton.addEventListener("click", () => {
  ...
  radarSound.stop();
  try {
    manageSabotageSound("stop", "car selection reset");
    tempoTempoSound.stop();
  } catch (e) {
...
startButton.addEventListener("click", async () => {
  ...
  console.log(
    "Testing game over sound state:",
    gameOverSound ? gameOverSound.state() : "undefined"
  );
